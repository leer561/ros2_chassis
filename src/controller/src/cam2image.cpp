...
int main(int argc, char *argv[])
{
    ...
    bool success = parse_command_options(argc, argv,
                                         &depth,
                                         &reliability_policy,
                                         &history_policy,
                                         &show_camera,
    ...
    // Depth represents how many messages to store in history when the history
    // policy is KEEP_LAST.
    custom_camera_qos_profile.depth = depth;

    // The reliability policy can be reliable, meaning that the underlying
    // transport layer will try ensure that every message gets received in order,
    // or best effort, meaning that the transport makes no guarantees about the
    // order or reliability of delivery.
    ...
    // taken by the reader. KEEP_ALL saves all messages until they are taken.
    ...
    // is_flipped will cause the incoming camera image message to flip about the
    // y-axis.
    bool is_flipped = false;

    // Subscribe to a message that will toggle flipping or not flipping, and
    // manage the state in a callback.
    ...
    auto sub = node->create_subscription<std_msgs::msg::Bool>("flip_image", callback, custom_flip_qos_profile);
