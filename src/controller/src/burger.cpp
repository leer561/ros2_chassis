...
void decode_base64(const char *cstr, std::vector<uint8_t> &out)
{
    ...
    for (ridx = 0; ridx < len; ridx += 4)
    {
        ...
        uint32_t block = 0;
        block =
            (base64_map[cstr[ridx]] << 18) | (base64_map[cstr[ridx + 1]] << 12) |
            (base64_map[cstr[ridx + 2]] << 6) | (base64_map[cstr[ridx + 3]] << 0);
...
cv::Mat &Burger::render_burger(size_t width, size_t height)
{
    ...
    if (width_i < burger_template.size().width ||
        height_i < burger_template.size().height)
    {
    ...
    if (burger_buf.size().width != width_i ||
        burger_buf.size().height != height_i)
    {
    ...
    for (int b = 0; b < static_cast<int>(x.size()); b++)
    {
        burger_template.copyTo(
            burger_buf(cv::Rect(x[b], y[b], burger_template.size().height,
